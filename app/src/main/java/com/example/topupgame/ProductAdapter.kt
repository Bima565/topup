...
class ProductAdapter(private val productList: List<Product>) :
    ...
    private fun updateFavoriteIcon(imageView: ImageView, isFavorite: Boolean) {
        if (isFavorite) {
            imageView.setImageResource(R.drawable.ic_favorite) // Pastikan ini icon hati penuh/merah
            imageView.setColorFilter(android.graphics.Color.RED)
        } else {
            imageView.setImageResource(R.drawable.ic_favorite) // Icon hati kosong atau outline
